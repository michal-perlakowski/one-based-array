...
import OneBasedArray from '../src/index';
...
describe('OneBasedArray', () => {
  ...
  describe('proxy handlers', () => {
    describe('get', () => {
      ...
      it('should work with symbols', () => {
        ...
        expect(arr[Symbol('symbol')]).to.be.undefined;
      });
    });
    describe('set', () => {
  ...
  describe('Symbol.iterator', () => {
    it('should iterate over all elements', () => {
      expect(Array.from(arr)).to.deep.equal(['a', 'b', 'c']);
    });
  });
  describe('Array methods', () => {
    describe('concat', () => {
      ...
      it('should concat OneBasedArray with non-array elements', () => {
        expect(arr.concat(true, null, 42, 'd', Symbol.for('test'), {})).to.deep
          .equal(new OneBasedArray('a', 'b', 'c', true, null, 42, 'd', Symbol.for('test'), {}));
      });
    });
    describe('includes', () => {
    ...
    describe('forEach, every, some, find, findIndex, filter, map', () => {
      it('should execute the function once for each element with this value equal to the provided thisArg', () => {
        ['forEach', 'every', 'some', 'find', 'findIndex', 'filter', 'map'].forEach((method) => {
    ...
    describe('keys, values, entries', () => {
      it('should return appropriate iterator object', () => {
        expect(Array.from(arr.keys())).to.deep.equal([1, 2, 3]);
        expect(Array.from(arr.values())).to.deep.equal(['a', 'b', 'c']);
        expect(Array.from(arr.entries())).to.deep.equal([[1, 'a'], [2, 'b'], [3, 'c']]);
      });
    });
  });
});
