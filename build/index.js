...
const convertMethod = (method, target) => {
  ...
  const transformArgsMap = {
    ...
    reduce: (callback, ...rest) => [(a, b, i, array) => callback(a, b, i + 1, Reflect.construct(target.constructor, array)), ...rest]
  };
  ...
  return (...args) => {
    ...
    if (transformArgsMap.hasOwnProperty(method)) {
      ...
      transformedArgs = [(currentValue, index, array) => Reflect.apply(args[0], args[1], [currentValue, index + 1, Reflect.construct(target.constructor, array)])];
    }
...
class OneBasedArray extends Array {
  constructor(...args) {
    super(...args);
    ...
    const convertHelper = (property, back = false) => {
      ...
      return !Number.isNaN(parsed) && Number.isFinite(parsed) && parsed >= 0 && parsed <= maxArrayLength ? back ? String(parsed + 1) : parsed - 1 : property;
    };
    ...
    return new Proxy(this, {
      get(target, property, receiver) {
        const methodList = ['concat', 'includes', 'join', 'slice', 'toString', 'toLocaleString', 'indexOf', 'lastIndexOf', 'copyWithin', 'fill', 'pop', 'push', 'reverse', 'shift', 'unshift', 'sort', 'splice', 'forEach', 'every', 'some', 'find', 'filter', 'findIndex', 'map', 'reduce', 'reduceRight', 'keys', 'values', 'entries'];
        return methodList.includes(property) ? convertMethod(property, target) : Reflect.get(target, convertProperty(property), receiver);
      },
